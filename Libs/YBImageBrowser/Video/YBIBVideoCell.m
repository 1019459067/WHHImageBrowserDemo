...
- (void)yb_pageChanged {
    if (self.yb_currentPage() != self.yb_selfPage()) {
        ...
        [self performSelector:@selector(playVideo) withObject:nil afterDelay:0.25];
    }
}
...
- (void)yb_respondsToTapGestureForVideoView:(YBIBVideoView *)view {
    ...
    if (data.singleTouchBlock) {
        ...
        if (UIDeviceOrientationIsPortrait(([YBIBScreenRotationHandler getDeviceOrientation]))) {
            if ([data isKindOfClass:YBIBVideoData.class] == NO) {
                [self hideBrowser];
            }
        } else {
            // nothing
...
- (void)yb_videoView:(YBIBVideoView *)view didClickedFullScreen:(BOOL)isFullScreen
{
    if ([self.delegate respondsToSelector:@selector(yb_videoViewDidClickedFullScreen:)]) {
        [self.delegate yb_videoViewDidClickedFullScreen:isFullScreen];
    }
}
...
- (void)respondsToPanGesture:(UIPanGestureRecognizer *)pan {
    if ([YBIBUtilities sharedInstance].bFullScreen == YES) {
        return;
    }
