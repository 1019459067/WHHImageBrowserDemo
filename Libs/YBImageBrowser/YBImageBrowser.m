...
@interface YBImageBrowser () <UICollectionViewDelegate, UICollectionViewDataSource, YBIBVideoCellDelegate>
...
- (instancetype)initWithDataSoure:(NSArray *)dataArray
...
{
    NSInteger videoPlayTime = 0;
    BOOL isPlaying = NO;
    if ([cell isKindOfClass:YBIBVideoCell.class]) {
        YBIBVideoCell *videoCell = (YBIBVideoCell *)cell;
        
        isPlaying = videoCell.videoView.isPlaying;
        CMTime time = videoCell.videoView.player.currentTime;
        videoPlayTime = (int)((time.value) / (CGFloat)time.timescale * 1000);
    }
...
- (UICollectionViewCell *)collectionView:(YBIBCollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([data isKindOfClass:YBIBVideoData.class]) {
        self.rotationHandler.supportedOrientations = UIInterfaceOrientationMaskPortrait|UIInterfaceOrientationMaskLandscapeLeft|UIInterfaceOrientationMaskLandscapeRight|UIInterfaceOrientationMaskPortraitUpsideDown;
        YBIBVideoCell *videoCell = (YBIBVideoCell *)cell;
        videoCell.delegate = self;
    } else {
        self.rotationHandler.supportedOrientations = UIInterfaceOrientationMaskPortrait|UIInterfaceOrientationMaskPortraitUpsideDown;
    }
...
#pragma mark - YBIBVideoCellDelegate
- (void)yb_videoViewDidClickedFullScreen:(BOOL)isFullScreen
{
    self.collectionView.scrollEnabled = !isFullScreen;
    [self.rotationHandler changeUIWithtOrientation:isFullScreen ? UIDeviceOrientationLandscapeLeft : UIDeviceOrientationPortrait];
}
